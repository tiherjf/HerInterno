...
interface Doc {
  ...
  version: string; status: string; requires_reading: boolean;
...
export default function DocumentosPage() {
  ...
  const [form, setForm] = useState({
    code: "", title: "", doc_type: "pop", category: "",
  ...
  async function createDoc() {
    ...
    setShowForm(false);
    setForm({ code:"",title:"",doc_type:"pop",category:"",file_url:"",requires_reading:false,valid_from:"",valid_until:"" });
  ...
  const grouped = Object.entries(DOC_TYPE_LABELS).map(([type, label]) => ({
    ...
    docs: docs.filter(d => {
      ...
      return d.doc_type === type && matchType && matchSearch;
    }),
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold flex items-center gap-2"><FileText size={22} className="text-emerald-500" /> Documentos Controlados</h2>
          <p className="text-muted-foreground text-sm">POPs, protocolos, políticas e manuais com controle de versão</p>
        </div>
        <Button onClick={() => setShowForm(true)}><Plus size={14} /> Novo Documento</Button>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-muted-foreground"><FileText size={36} className="mx-auto mb-3 opacity-30" /><p>Nenhum documento encontrado.</p></div>
      ) : (
        <div className="space-y-6">
                ...
                {gDocs.map(doc => (
                  <button key={doc.id} onClick={() => setSelected(doc)}
                    ...
                    <div className="flex items-center gap-3 text-xs text-muted-foreground">
                      {doc.valid_until && (
                        <span className={`flex items-center gap-1 ${isExpiring(doc) ? "text-amber-600 font-medium" : ""}`}>
                          <Clock size={10} /> Válido até {fmt(doc.valid_until)}
                        </span>
                      )}
                      {doc.requires_reading && (
                        doc.user_has_read
                          ? <span className="flex items-center gap-1 text-green-600"><CheckCircle2 size={10} /> Leitura confirmada</span>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-xl">
          ...
          <div className="space-y-3 pt-2">
            ...
            <div><Label className="text-xs">Categoria</Label><Input value={form.category} onChange={e => setForm({...form, category: e.target.value})} placeholder="Ex: Enfermagem, Farmácia..." /></div>
              ...
              <div className="space-y-4 pt-2">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><p className="text-xs text-muted-foreground">Status</p>
                    <Badge className={`text-xs border-0 mt-1 ${STATUS_META[selected.status]?.color}`}>{STATUS_META[selected.status]?.label}</Badge>
                  </div>
                  {selected.category && <div><p className="text-xs text-muted-foreground">Categoria</p><p className="font-medium">{selected.category}</p></div>}
                ...
                {selected.requires_reading && (
                  <div className={`rounded-lg p-3 border ${selected.user_has_read ? "bg-green-50 border-green-100" : "bg-orange-50 border-orange-100"}`}>
                    {selected.user_has_read ? (
                      <p className="text-sm text-green-700 flex items-center gap-2"><CheckCircle2 size={16} /> Você confirmou a leitura deste documento.</p>
                    ) : (
                      <div>
