...
export default function AuditoriasPage() {
  ...
  const [findingForm, setFindingForm] = useState({ finding_type: "nc", description: "", sector: "", create_nc: false });
  const [findingSaving, setFindingSaving] = useState(false);

  const [form, setForm] = useState({
    title: "", audit_type: "interna", auditor_external: "", scope: "", audit_date: "",
  });
  ...
  async function addFinding(auditId: string) {
    ...
    setFindingForm({ finding_type: "nc", description: "", sector: "", create_nc: false });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold flex items-center gap-2"><ClipboardList size={22} className="text-purple-500" /> Auditorias</h2>
          <p className="text-muted-foreground text-sm">Agende auditorias, registre achados e vincule a planos de ação</p>
        </div>
        <Button onClick={() => setShowForm(true)}><Plus size={14} /> Nova Auditoria</Button>
      </div>

      {loading ? (
        <div className="flex justify-center py-12"><Loader2 className="animate-spin text-muted-foreground" /></div>
      ) : audits.length === 0 ? (
        <div className="text-center py-12 text-muted-foreground"><ClipboardList size={36} className="mx-auto mb-3 opacity-30" /><p>Nenhuma auditoria cadastrada.</p></div>
      ) : (
          ...
          {audits.map(audit => (
            <button key={audit.id} onClick={() => openDetail(audit.id)}
              ...
              <div className="shrink-0">
                <Badge className={`text-xs border-0 ${audit.audit_type === "interna" ? "bg-purple-100 text-purple-700" : "bg-orange-100 text-orange-700"}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex flex-wrap gap-x-4 mt-0.5 text-xs text-muted-foreground">
                  ...
                  {audit.auditor && <span>{audit.auditor.full_name}</span>}
                  {audit.auditor_external && <span>{audit.auditor_external}</span>}
                </div>
              </div>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-xl">
          ...
          <div className="space-y-3 pt-2">
            ...
            <div><Label className="text-xs">Auditor externo (se externo)</Label><Input value={form.auditor_external} onChange={e => setForm({...form, auditor_external: e.target.value})} placeholder="Nome da empresa/pessoa" /></div>
            <div><Label className="text-xs">Escopo</Label><Textarea rows={2} value={form.scope} onChange={e => setForm({...form, scope: e.target.value})} placeholder="O que será auditado..." /></div>
            ...
            <>
              <DialogHeader>
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <DialogTitle>{selected.audit.title}</DialogTitle>
              ...
              <div className="space-y-5 pt-2">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  {(selected.audit.auditor || selected.audit.auditor_external) && (
                    <div><p className="text-xs text-muted-foreground">Auditor</p><p className="font-medium">{selected.audit.auditor?.full_name ?? selected.audit.auditor_external}</p></div>
                  )}
                ...
                <div>
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <Button size="sm" variant="outline" onClick={() => setAddingFinding(v => !v)}><Plus size={12} /> Registrar achado</Button>
                  </div>
                  ...
                  {addingFinding && (
                    <div className="bg-purple-50 border border-purple-200 rounded-lg p-4 mb-3 space-y-3">
                      <div className="grid grid-cols-2 gap-2">
                        ...
                        <div><Label className="text-xs">Setor</Label><Input value={findingForm.sector} onChange={e => setFindingForm({...findingForm, sector: e.target.value})} /></div>
                      </div>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {f.nc && (
                                <p className="text-xs mt-1 text-muted-foreground">
                                  NC vinculada: <span className="font-mono">{f.nc.number}</span> — {f.nc.title}
                                </p>
                              )}
                            </div>
                          </div>
